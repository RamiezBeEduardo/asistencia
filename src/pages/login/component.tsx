...
const component = () => {
    ...
    if (planes && planes.dato) {
        planes_options = planes.dato.map((item: any) => {
            return <Select.Option key={item.crrcodi} value={item.crrcodi}>{item.crrcodi}</Select.Option>
        })
    }

    if (cursos && cursos.dato) {
        cursos_options = cursos.dato.map((item: any) => {
            return <Select.Option key={item.curcodi} value={item.curcodi}>{item.curnomb}</Select.Option>
        })
    }

    if (secciones && secciones.dato) {
        seccion_options = secciones.dato.map((item: any) => {
            return <Select.Option key={item.seccodi} value={item.seccodi}>{item.seccion}</Select.Option>
        })
    }
            ...
            >
                ...
                >
                    ...
                    <Form.Item name="plan" label="Plan" rules={[{ required: true }]}>
                        <Select onSelect={ planChange }>
                            { planes_options }
                        </Select>
                    </Form.Item>

                    <Form.Item  name="curso" label="Curso" rules={[{ required: true }]}>
                        <Select onSelect={ cursoChange }>
                            { cursos_options }
                        </Select>
                    </Form.Item>

                    <Form.Item  name="seccion" label="Seccion" rules={[{ required: true }]}>
                        <Select onSelect={ seccionChange }>
                            { seccion_options }
                        </Select>
                    </Form.Item>
    ...
    const handleClick = (item) => {
        ...
        console.log(alumnos)
        console.log(item)
        let newalumnos = alumnos.filter((alumno) => { return item.codigoAlumno != alumno.codigoAlumno})
